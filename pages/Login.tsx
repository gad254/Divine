...
import { Flame, Mail, Lock, AlertCircle, ChevronDown } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLogin, onNavigateSignup, lang, setLang }) => {
  ...
  return (
    <div className="h-full bg-white flex flex-col px-6 md:px-8 py-6 md:pt-12 md:pb-8 relative overflow-y-auto">
      {/* Language Selector */}
      ...
      <div className="flex flex-col items-center mb-8 md:mb-12 shrink-0">
        <div className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-tr from-primary to-secondary rounded-full flex items-center justify-center text-white shadow-lg mb-4">
          <Flame size={28} md:size={32} fill="currentColor" />
        </div>
        <h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tighter">Divine</h1>
      </div>

      {/* Main Content */}
      <div className="flex-1 flex flex-col justify-center min-h-[400px]">
        <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">{t.welcome}</h2>
        <p className="text-sm md:text-base text-gray-500 mb-6 md:mb-8">{t.subtitle}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full bg-gradient-to-r from-primary to-secondary text-white font-bold py-3.5 md:py-4 rounded-full shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all mt-6 ${loading ? 'opacity-70 cursor-wait' : ''}`}
          >
            {loading ? '...' : t.login}
          </button>
        </form>
